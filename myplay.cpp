...
#define soundSpeed 0.1
void addBeat(double freq){
    int i;
    int dens=4;
    double oqs=0.2;
    for(i=play_n;i<play_n+dens;i++){
        afr[i]=1./dens;
        fr[i]=freq*pow(2,oqs*(i-play_n+1)/((double)dens));
        fr1[i]=fr[i];
        ufr[i]=0;
        panfr[i]=0.5;
        pfr[i]=0;
...
void playMyPlay(){
    ...
    double     reverb=calcOpt("myplay_reverb");
...
// }
 ...
 initmysin();
 if(GetTickCount()-lastCalcPlay>1){
    lastCalcPlay=GetTickCount();
    if(getOpt("myplay_mode")<0.333){
         hearZones();   
    }else if(getOpt("myplay_mode")<0.6666){
         playSint2();
    }
 
 }
 if(getOpt("myplay_mode")>=0.6666){
         s2_frn=0;
//         fprintf(stderr,"play_n: %d .. ",play_n);
         doRtm();
  //       fprintf(stderr," .. %d .. ",play_n);
         s2fr2fr();
    //     fprintf(stderr,".. %d\n",play_n);
    }
  ...
  int loops=0;
  if(getOpt("myplay_metod")>0.66){
        //metod2();
//    setOpt("myplay_metod",0);
//    addBeat(50);
//    addBeat(100);
//    addBeat(150);
  }else if(getOpt("myplay_metod")>0.33){
  }else {
      while((pldata.b+1)%PLAYBUFLEN!=pldata.e){
        while(locked);  
        ...
        for(i=0;i<play_n;i++){
            ...
            afr[i]=afr[i]*(1-soundSpeed)+afr1[i]*soundSpeed;
            pfr[i]=pfr[i]*(1-soundSpeed)+pfr1[i]*soundSpeed;

            
            fr[i]=fr[i]*(1-soundSpeed)+fr1[i]*soundSpeed;
...
//    fprintf(stderr,"clc:%d .. ",s2_frn);
    playSin2freq(1,400,1,0.5,1,1,0,0,0,0,0.3,n,1,1);
//    fprintf(stderr," .. %d\n",s2_frn);
...
/*        for(j=i+1;j<s2_frn;j++){
        ...
        if(fou>=0){
            ...
            s2_frtime[i]=max(frtime[fou],s2_frtime[i]);
