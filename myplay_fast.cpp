...
void hearZones();
#define PLAY_N 1024
unsigned int panfr[PLAY_N]={0.5,.5,.5,.5,.5,.5,.5,.5,.5,0.5};
unsigned int pfr[PLAY_N]={0.0,.0,.0,.01,.05,.07,.08,.09,.095,1};
unsigned int pfr1[PLAY_N]={0.0,.0,.0,.01,.05,.07,.08,.09,.095,1};
unsigned int afr[PLAY_N]={0.0,.0,.0,.01,.05,.07,.08,.09,.095,1};
unsigned int afr1[PLAY_N]={0.0,.0,.0,.01,.05,.07,.08,.09,.095,1};

unsigned int fr[PLAY_N]={50,100,150,200,250,300,350,400,450,500};
unsigned int fr1[PLAY_N]={50,100,150,200,250,300,350,400,450,500};


unsigned int frid[PLAY_N];


 unsigned int ufr[PLAY_N]={0};
 unsigned int dphfr[PLAY_N]={0};
 
 unsigned int afr_tre[PLAY_N];
 unsigned int fr_tre[PLAY_N];
 unsigned int ufr_tre[PLAY_N];

 unsigned int afr_vib[PLAY_N];
 unsigned int fr_vib[PLAY_N];
 unsigned int ufr_vib[PLAY_N];
...
unsigned int *afr_vib;
unsigned int *fr_vib;
unsigned int *ufr_vib;

unsigned int *afr_tre;
unsigned int *fr_tre;
unsigned int *ufr_tre;


unsigned int *dphfr;
unsigned int*simpleVol;
...
void initmysin(){
    ...
    initRnd();
    for(i=0;i<65536;i++)
        sinBuf[i]=32767*sin(i*2*M_PI/PLAY_SAMPLE_RATE);
    int j;
 ...
 int mysin(Uint16 a){
//    return sin(a*2.*M_PI/PLAY_SAMPLE_RATE)*32000;
    return sinBuf[a];

}
double fl=100;
double lsv=0;
double maxSimpleVol=1;
...
// }
 ...
 /* double nsv=getOpt("myplay_simpleVol");
  ...
  lsv=getOpt("myplay_simpleVol");
 
  simpleVol=10*lsv;
  int loops=0;
  if(getOpt("myplay_metod")>0.5){
//        metod2();
    fprintf(stderr,"No realize of metod2\n");
  }else{
      while((pldata.b+1)%PLAYBUFLEN!=pldata.e){
        while(locked);  
        ...
        int i;
        int vr,vl,v,v_vib,v_tre;
        vr=0;
        vl=0;
        for(i=0;i<play_n;i++){
            
            v=((int)afr[i])*mysin(ufr[i])/32768;///fr[i];
            
//            ufr_tre[i]+=(fr_tre[i]);

  //          v_tre=32767+afr_tre[i]*mysin(ufr_tre[i])/32768;

            
            
            ufr[i]+=fr[i];//*v_tre*rnd(pfr[i]))/SQPLSR;
            
            afr[i]=afr1[i];//(afr[i]*1023 + afr1[i])/1024;
            pfr[i]=pfr1[i];//(pfr[i]*1023 + pfr1[i])/1024;
            fr[i]=fr1[i];//(fr[i]*1023 + fr1[i])/1024;
    ...
    //        ufr_vib[i]+=fr_vib[i];
      ...
      //      v_vib=afr_vib[i]*mysin(ufr_vib[i])/32768;

//            v=simpleVol*(v*(32768+v_vib)/32768)/32768;
            v=simpleVol*v/32768;
            vr+=v;//*panfr[i]/65536;
            vl+=v;//*(65535-panfr[i])/65536;
            
            
        }
        if(vr>32766||vr<-32766||vl>32766||vl<-32766){
//            simpleVol=simpleVol*127/128;
  //          svupdate=1;
          fprintf(stderr,"CLIP!\n");     
       }
        if(vr>32767)
...
int s2_frn;
unsigned int s2_afr[PLAY_N]={0.0,.0,.0,.01,.05,.07,.08,.09,.095,1};
unsigned int s2_afr0[PLAY_N]={0.0,.0,.0,.01,.05,.07,.08,.09,.095,1};

unsigned int s2_pfr[PLAY_N]={0.0,.0,.0,.01,.05,.07,.08,.09,.095,1};
unsigned int s2_pfr0[PLAY_N]={0.0,.0,.0,.01,.05,.07,.08,.09,.095,1};

unsigned int s2_fr[PLAY_N]={50,100,150,200,250,300,350,400,450,500};
unsigned int s2_fr0[PLAY_N]={50,100,150,200,250,300,350,400,450,500};

unsigned int s2_frid[PLAY_N];
unsigned int s2_ufr[PLAY_N]={0};

unsigned int s2_afr_vib[PLAY_N]={0.0,.0,.0,.01,.05,.07,.08,.09,.095,1};
unsigned int s2_fr_vib[PLAY_N]={50,100,150,200,250,300,350,400,450,500};
unsigned int s2_ufr_vib[PLAY_N]={0};

unsigned int s2_afr_tre[PLAY_N]={0.0,.0,.0,.01,.05,.07,.08,.09,.095,1};
unsigned int s2_fr_tre[PLAY_N]={50,100,150,200,250,300,350,400,450,500};
unsigned int s2_ufr_tre[PLAY_N]={0};


unsigned int s2_panfr[PLAY_N]={0.5,.5,.5,.5,.5,.5,.5,.5,.5,0.5};
...
//    (f1*a1+f1*a2+dfr*a2)=freq*(a1+a2);
    ...
    s2_afr[s2_frn]=a1*32767;
    s2_pfr[s2_frn]=pfr;
    
    s2_fr[s2_frn]=f1;//*PLAY_SAMPLE_RATE/2/M_PI;
    s2_panfr[s2_frn]=pan*65536;
    s2_frid[s2_frn]=nsint+ 12*ngar +noq*12*16;
    
    s2_afr_vib[s2_frn]=a_vib*32767;
    s2_fr_vib[s2_frn]=fr_vib*65536/2/M_PI;
 
    s2_afr_tre[s2_frn]=a_tre*32767;
    s2_fr_tre[s2_frn]=fr_tre*65536/2/M_PI;
...
//    if(a2>0.000001){
        s2_afr[s2_frn]=a2*32757;///*kos;
        s2_pfr[s2_frn]=pfr;///*kos;
       
        s2_fr[s2_frn]=f2;//*PLAY_SAMPLE_RATE/2/M_PI;
        s2_panfr[s2_frn]=pan*65536;
        s2_afr_vib[s2_frn]=a_vib*32767;
        s2_fr_vib[s2_frn]=fr_vib*65536/2/M_PI;
...
/*        for(j=i+1;j<s2_frn;j++){
        ...
        fou=-1;
        for(j=0;j<play_n;j++){
            if(s2_frid[i]== frid[j] && s2_fr[i]==fr[j]  && !ufr_used[j]){
                dfr=ddd;
                fou=j;
                break;                
            }
  ...
  // locked=1;
    play_n=0;
    for(i=0;i<s2_frn;i++){
        if((s2_afr[i]>1 || s2_afr0[i]>1) && ((s2_fr[i]>16)|| (s2_fr0[i]>16))){
