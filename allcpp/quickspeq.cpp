...
double calcSumQSDis(struct qs*q,double st,double fin){
    double r;
    int i,j,n,n0,n1;
    r=0;
    n0=st*q->oqs*q->tones;
    n1=fin*q->oqs*q->tones;
    for(n=0,j=0;j<q->oqs;j++){
        for(i=0;i<q->tones;i++,n++){
            if(n<n0)
                continue;
            if(n>n1)
                break;
...
double calcSumQSDis(struct qs*q){
    return calcSumQSDis(q,0,1);
}



double calcSumQSMm(struct qs*q,double st,double fin){
    double r;
    int i,j,n,n0,n1;
    r=0;
    n0=st*q->oqs*q->tones;
    n1=fin*q->oqs*q->tones;
    for(j=3,n=0;j<q->oqs;j++){
        for(i=0;i<q->tones;i++,n++){
            if(n<n0)
                continue;
            if(n>n1)
...
int  reallocQs(struct qs*q,int tones,int oq){
    if(q->ks==NULL){
        ...
        q->badKs=(double*)malloc(tones*oq*sizeof(double));
        if(q->badKs==NULL)
            return -1;
        memset(q->badKs,0,tones*oq*sizeof(double));
        ...
        q->badSns=(struct sense*)malloc(tones*oq*sizeof(struct sense));
        if(q->badSns==NULL)
            return -1;
        zeroSenses(q->badSns,tones*oq,"badKs");
    ...
    if(tones*oq>q->bufsize){
        ...
        if(q->badSns==NULL)
            return -1;
        zeroSenses(q->sns,tones*oq,"badKs");
...
void setQs(struct qs*r,int t,int oq,double value){
 ...
 //   soft
    ...
    r->ks[adr]=pow(senseValue(&(r->sns[adr]),value),quickSpeqAmpGamma);
}
void setBadQs(struct qs*r,int t,int oq,double value){
    int adr;
    if(t>=r->tones || oq>=r->oqs ||t<0||oq<0)
        return;
 ...
 //   soft
    ...
    r->badKs[adr]=senseValue(&(r->badSns[adr]),value);
}
...
void speq2qs(double*sound_a,double*freqs,int n,int tones,int minoq,int maxoq,struct qs*r){
    ...
    int lt,loq;
    double like;
    quickSpeqAmpGamma=getOpt("quickSpeqAmpGamma");
              ...
              //      if(startSkip!=-1000){
                //           for(int i=startSkip;i<startSkip-1/*la*/;i++){
                 ...
                 //   }
                    ...
                    //r->sns[]soft
                    setQs(r,lt,loq-minoq,v+badsum);
                    setBadQs(r,lt,loq-minoq,badsum);
...
//                    for(int i=startSkip;i<startSkip-1/*la*/;i++){
    ...
    double near;
    near=getOpt("nearSoundArrays");
    nearbySenses(r->sns,r->oqs*(int)r->tones,1./12.*near);

    nearbySenses(r->snsMm,r->oqs*(int)r->tones,1./12.*near);
    nearbySenses(r->snsMm05,r->oqs*(int)r->tones,1./12.*near);
    nearbySenses(r->snsDis,r->oqs*(int)r->tones,1./12.*near);
  //  for(int i=0;i<24;i++){
...
void speq2qs(double*sound_a,double*freqs,double*b_sound_a,double*b_freqs,int n,int tones,int minoq,int maxoq,struct qs*r,int calcQS){
    int i;
    double summ,skipValue;
    int col,t,oq,startSkip;
    int lt,loq;
...
//int once=0;
void writeFreqs(double* qs_ft,int c,const char*name){
    FILE*f;
  //  if(once)
    //    return;
//    once=1;
    f=fopen(name,"w");
    if(f==NULL){
        fprintf(stderr,"%s cannot be oppen\n",(char*)name);
...
void writeFreqs(double* qs_ft,int c){
    writeFreqs(qs_ft,c,"freqs.txt");

}

void speq2qs(double*sound_a,double*freqs,double*b_sound_a,double*b_freqs,double*c_sound_a,double*c_freqs,int n,int tones,int minoq,int maxoq,struct qs*r,int calcQS){
    int i;
    double summ,skipValue;
    int col,t,oq,startSkip;
    int lt,loq;
...
void speq2qs(double*sound_a,double*freqs,double*b_sound_a,double*b_freqs,double*c_sound_a,double*c_freqs,double*d_sound_a,double*d_freqs,int n,int tones,int minoq,int maxoq,struct qs*r,int calcQS){
    int i;
    double summ,skipValue;
    int col,t,oq,startSkip;
    int lt,loq;
