...
inline void abcdSPEQ(int&b_cur,int&b_k,double&b_sum,ShortComplex*sound_arr,int sound_k,int&b_sound_k,ShortComplex*b_sound_arr,ShortComplex*b2_sound_arr,
...
void recHeader(FILE*f){
   // Содержит символы "RIFF" в ASCII кодировке
// (0x52494646 в big-endian представлении)
    unsigned int SIZE=0xffffffffffffff00;
    wh.chunkId[0]='R';
    wh.chunkId[1]='I';
    wh.chunkId[2]='F';
    wh.chunkId[3]='F';
...
inline void recordAudio(short r,short l){
    if(recAudio){
        if(recAudioFile==NULL){
            recAudioFile=fopen("recvideo/one.wav","wb");
            if(recAudioFile!=NULL){
...
//            recordAudio(data.buff1[data.begin],data.buff2[data.begin]);
    ...
    //return 0;
   double maxValue;
   if(getOpt("hardFrameDrop")>0.5 &&  (data.end-data.begin+data.len)%data.len>N){
        sound_k=0;
        continue;
    }
    if(sound_k==N){
