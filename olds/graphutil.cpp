...
extern volatile int recVideoImgReady;
extern volatile int recVideoImgW;
extern volatile int recVideoImgH;
extern volatile Uint32* recVideoImgPtr;
...
void sendFrameToRec(){
  ...
  if(recVideoImgPtr==NULL){
    recVideoImgPtr=(Uint32*)malloc(GLW*GLH*3);
    recVideoImgW=GLW;
    recVideoImgH=GLH;
  }
  if(recVideoImgPtr!=NULL&&    (recVideoImgW!=GLW||
    recVideoImgH!=GLH)){
  ...
  if(recVideoImgPtr==NULL)
     return;  
    glReadPixels(0, 0, GLW, GLH, GL_BGR, GL_UNSIGNED_BYTE,(unsigned int *) recVideoImgPtr);
    recVideoImgReady=1;
}


#define GLTEXDELAYN 100
...
double videoZoom,videoAspect,videoPosX,videoPosY;
...
void placeTexOnGlScreen(void*p,int h,int w,double alpha){
...
//          double alpha;
          ...
          double x1,y1,x2,y2;
          zoom=(videoZoom-0.5);
          zoomTang=(videoAspect-0.5);

          zoomx=zoom+zoomTang;
          zoomy=zoom-zoomTang;
          
          
          x1=zoomx+videoPosX-0.5;
          y1=zoomy+videoPosY-0.5;
          x2=videoPosX+0.5-zoomx;
          y2=videoPosY+0.5-zoomy;
...
void GLShowScreen(SDL_Surface* screen){
  //        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    //    glDisable(GL_DEPTH_TEST);
       ...
       videoZoom=calcOpt("videoZoom");
       videoAspect=calcOpt("videoAspect");
       videoPosX=calcOpt("videoPosX");
       videoPosY=calcOpt("videoPosY");
...
//double lglOpt[GLOPTS];
      ...
      double vfop1=calcOpt("videoFile1");
    double vfop2=calcOpt("videoFile2");
    double vfop3=calcOpt("videoFile3");
    if(vfop1+vfop2+vfop3>0.00001){
            int fr;
            fr=getFrame(&video,&videoh,&videow);
            double vfsize=0;
...
//.            mixBWSizedOrGL
            if(vfsize<=0.0001){
                if(fr==0)
                    ...
                    placeTexOnGlScreen(NULL,videoh,videow,vfop1);
...
//        getFrame(&video,&videoh,&videow);
            ...
            if(vfsize<=0.0001){
                placeTexOnGlScreen(NULL,videoh,videow,vfop3);
            }else{
    ...
    if(vfsize>0.00001){
                double op=calcOpt("pifInterOp4");
                double op2=calcOpt("pifInterOp4_q");
