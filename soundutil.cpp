...
//  #define N 8192
 ...
 // #define N 1024
  #define N 1024
  #define NK 10
  ShortComplex sound_arr[2048];
  double sound_a[2048];
  double sound_fi[2048];
  double sound_freq[2048];  
...
struct sense sensKoefsAr16[KOEFNUM][16];    
double koefsAr16[KOEFNUM][16];
...
void calcKoefAr16(){
    ...
    double ar[16];
    for(int i=0;i<1;i++){
        if(lifeMaps[i].w*lifeMaps[i].h!=0){
            life2arr(lifeMaps+i,1,ar);
            for(int j=0;j<16;j++){
                koefsAr16[i][j]=senseValue(&(sensKoefsAr16[i][j]),ar[j]);
//                fprintf(stderr,"%1.2lf ",koefsAr16[i][j]);
...
void setSesorsSpeed(){
    ...
    for(int i=0;i<KOEFNUM;i++){
        ...
        up=getOpt("speedKoefUp",KOEFNUM+i);
        dw=getOpt("speedkoefDown",KOEFNUM+i);
        on=getOpt("correctorKoef",KOEFNUM+i);
        for(j=0;j<16;j++){
            sensKoefsAr16[i][j].softup=up;
            sensKoefsAr16[i][j].soft=dw;
            sensKoefsAr16[i][j].c.on=on;
        }
    }
...
double findEqValueChromaSin(double *freqs,double *amps,double*Si,double*Co,double*Le,int n,double st,double fin){
    double f0;
    double f1;
    double maxfr;
    double eps;
    eps=0.;
    f0=mel2gz( gz2mel(freqs[n-1])   *st);
    f1=mel2gz( gz2mel(freqs[n-1])   *fin);
    double r,rdiv;
    int i;
    r=0;
    rdiv=0;
...
double findEqValueChromaCos(double *freqs,double *amps,double*Si,double*Co,double*Le,int n,double st,double fin){
    double f0;
    double f1;
    double maxfr;
    double eps;
...
double findEqValueChromaPitch(double *freqs,double *amps,int n,double st,double fin){
    double f0;
    double f1;
    double maxfr;
    double eps;
...
//                lastks4reverb=(double*)malloc(sizeof(double)*freqlen);
                ...
                memcpy(lastks4reverb,internalfreqs,freqlen*sizeof(double));    
            }
...
//                        (struct ssp*,double st,double fin){
       ...
       int tithmnumv2;
        ...
        for(ti=0;ti<KOEFNUM;ti++){
            ...
            if(fabs(getOpt("typeKoef",ti) - 8 )<0.5){
    //            fprintf(stderr,"Si\n");

                koefs[ti]=senseValue(sensKoefs+ti,findEqValueChromaSin(ssp1.freqs,ssp1.amps,ssp1.chromaSin,ssp1.chromaCos,ssp1.chromaLen,ssp1.n,getOpt("freqKoefStart",ti),getOpt("freqKoefFin",ti)));        
            }
            if(fabs(getOpt("typeKoef",ti) - 9 )<0.5){
  //              fprintf(stderr,"Co\n");

                koefs[ti]=senseValue(sensKoefs+ti,findEqValueChromaCos(ssp1.freqs,ssp1.amps,ssp1.chromaSin,ssp1.chromaCos,ssp1.chromaLen,ssp1.n,getOpt("freqKoefStart",ti),getOpt("freqKoefFin",ti)));        
                
                
                
            }

            if(fabs(getOpt("typeKoef",ti) - 10 )<0.5){
      //          fprintf(stderr,"findEqValueChromaPitch\n");
                koefs[ti]=senseValue(sensKoefs+ti,findEqValueChromaPitch(sound_freq,sound_a,ssp1.n,getOpt("freqKoefStart",ti),getOpt("freqKoefFin",ti)));        
            }

            if(fabs(getOpt("typeKoef",ti) - 11 )<0.5){
                double co,si,v;
//                fprintf(stderr,"Co+si\n");

                co=findEqValueChromaCos(ssp1.freqs,ssp1.amps,ssp1.chromaSin,ssp1.chromaCos,ssp1.chromaLen,ssp1.n,getOpt("freqKoefStart",ti),getOpt("freqKoefFin",ti));
                si=findEqValueChromaSin(ssp1.freqs,ssp1.amps,ssp1.chromaSin,ssp1.chromaCos,ssp1.chromaLen,ssp1.n,getOpt("freqKoefStart",ti),getOpt("freqKoefFin",ti));
                v=sqrt(co*co+si*si);        
            ...
            if(fabs(getOpt("typeKoef",ti) - 7.)<0.5){
                    ...
                    double v;
//                    fprintf()
                    v=predict(tithmnumv2++,koefs[(int)getOpt("koefRithmChose",ti)],getOpt("koefRithmMinFreq",ti),getOpt("koefRithmMaxFreq",ti),getOpt("koefRithmLen",ti),0.2);
                    
                    koefs[ti]=v;//senseValue(sensKoefs+ti,v);        
...
int calcSound_new(double*koefs){
    if(!initSensKoefs){
        initSensKoefs=1;
        zeroSenses(sensKoefs,KOEFNUM,"mainSensors",1);
        for(i=0;i<KOEFNUM;i++){
            zeroSenses(sensKoefsAr16[i],16,"ar16Sensors",1);
        }
    }
    int cop=0;
    do{
    while(data.begin!=data.end){
        ...
        v=data.buff1[data.begin]/2.+data.buff2[data.begin]/2.;
...
/*        */
        ...
        if((data.begin)==data.end){
            //data.begin=(data.begin+data.len-1)%data.len;
        ...
        signal[signalCur]=v;
        signal[signalCur+2048]=v;
        signalCur++;
        signalCur%=2048;
    }
    }
    while(cop<2048&&!onceCalced);
    
    int sigbeg;
    sigbeg=(signalCur+1)%2048;
    for(i=0;i<2048;i++){
        sound_arr[i].re=signal[i+sigbeg];
        sound_arr[i].im=0;
    }  
        fft(sound_arr, 11, false);
        
        Nmax= (2048 + 1) / 2;
        
        for(i=0;i<Nmax;i++){
            sound_a[i]=sqrt(sound_arr[i].re*sound_arr[i].re + sound_arr[i].im*sound_arr[i].im)/2048;
            sound_freq[i]= (SAMPLE_RATE/2 *(double) i) / 2048;
    ...
    //        if (sound_fi[i] > M_PI)
      //          sound_fi[i]-= 2.*M_PI;
        ...
        if(!onceFreqs){
            ...
            speq2qs(sound_a,sound_freq,Nmax,12,-3,6,&qsAll);    
...
//    playMyPlay();
  ...
  int b_cur=0;
    ...
    */
    soundLoop++;
    if(getOpt("myplay")>0.5&&soundLoop>2)
        break;
...
//                fprintf(stderr,"w2s\n");
    ...
    //return 0;
   double maxValue;
   if(sound_k<N &&sound_k>1 && onceCalced &&  getOpt("hardFrameDrop")>0.5 &&  (data.end-data.begin+data.len)%data.len>N){
        sound_k=0;

        fprintf(stderr,"hardFrameDrop (a)\n");
//        continue;
    }
  /*  if(b_sound_k<N && b_sound_k>1 && onceCalced &&  getOpt("hardFrameDrop")>0.5 &&  (data.end-data.begin+data.len)%data.len>N*b_k){
        b_sound_k=0;
        b_cur=0;
        b_sum=0;
        fprintf(stderr,"hardFrameDrop(b)\n");
     //   continue;
    }
    if(c_sound_k<N &&c_sound_k>1 && onceCalced &&  getOpt("hardFrameDrop")>0.5 &&  (data.end-data.begin+data.len)%data.len>N*c_k){
        c_sound_k=0;
        c_cur=0;
        c_sum=0;
        fprintf(stderr,"hardFrameDrop(b)\n");
...
*///    if(d_sound_k<N && onceCalced &&  getOpt("hardFrameDrop")>0.5 &&  (data.end-data.begin+data.len)%data.len>N*d_k){
  //      d_sound_k=0;
    //    fprintf(stderr,"hardFrameDrop(b)\n");
      //  continue;
   // }
...
//        loadFreqs2frafr(sound_freq,sound_a,Nmax);

//        double*sound_a,double*freqs,int n,int tones,int minoq,int maxoq,struct qs*r
        if(b_ready&&c_ready/*&&d_ready*/||superfast){
            if(!onceFreqs){
                onceFreqs=1;
                writeFreqs(sound_freq,Nmax,"freqsa.txt");
...
//                speq2qs(sound_a,sound_freq,b_sound_a,b_sound_freq,c_sound_a,c_sound_freq/*,d_sound_a,d_sound_freq*/,Nmax,12,-3,6,&qsAll,1);    
                if(!superfast){
                      speq2qs(sound_a,sound_freq,b_sound_a,b_sound_freq,c_sound_a,c_sound_freq,c_sound_a,c_sound_freq,Nmax,12,-3,6,&qsAll,1);    
                }else{
                      speq2qs(sound_a,sound_freq,Nmax,12,-3,6,&qsAll);    
            ...
            ||getOpt("soundSpeqSquare1") >0.0001||getOpt("soundSpeqSquare2") >0.0001||getOpt("soundSpeqSquare3") >0.0001
           ...
           //     calcSoundSpeq(ssp1,sound_a,sound_freq,Nmax);
                if(!superfast){
                  ...
                  //  speq2qs(sound_a,sound_freq,Nmax,12,-3,6,&qsAll,0);    
                    calcSoundSpeq(ssp1,sound_a,sound_freq,Nmax);
                
                }
         //           ssp2frafr(ssp1);

  //                 calcSoundSpeq(ssp1,b_sound_a,b_sound_freq,Nmax);
...
//      if(soundLoop>2)
  //          break;
    ...
    if(soundControllFlag && (b_ready&&c_ready/*&&d_ready*/ ||  superfast)){
     /*   koefs[0]=bassrithm;
        koefs[1]=melodyrithm;
        koefs[2]=allrithm;
   //     koefs[3]=mtarget;
...
/*            if(fabs(getOpt("typeKoef",ti) - 8 )<0.5){
                koefs[ti]=senseValue(sensKoefs+ti,findEqValueChromaSin(sound_freq,sound_a,Nmax,getOpt("freqKoefStart",ti),getOpt("freqKoefFin",ti)));        
            }
            if(fabs(getOpt("typeKoef",ti) - 9 )<0.5){
                koefs[ti]=senseValue(sensKoefs+ti,findEqValueChromaCos(sound_freq,sound_a,Nmax,getOpt("freqKoefStart",ti),getOpt("freqKoefFin",ti)));        
            }*/

            if(fabs(getOpt("typeKoef",ti) - 10 )<0.5){
                koefs[ti]=senseValue(sensKoefs+ti,findEqValueChromaPitch(sound_freq,sound_a,Nmax,getOpt("freqKoefStart",ti),getOpt("freqKoefFin",ti)));        
            }
/*
            if(fabs(getOpt("typeKoef",ti) - 11 )<0.5){
                double co,si,v;
                co=findEqValueChromaCos(sound_freq,sound_a,Nmax,getOpt("freqKoefStart",ti),getOpt("freqKoefFin",ti));
                si=findEqValueChromaSin(sound_freq,sound_a,Nmax,getOpt("freqKoefStart",ti),getOpt("freqKoefFin",ti));
                v=sqrt(co*co+si*si);        
                koefs[ti]=senseValue(sensKoefs+ti,v);
