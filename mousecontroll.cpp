...
inline bool operator==(struct adr const& a,struct adr const& b){
...
void addEValue(int x1,int y1,int x2,int y2,struct adr a,struct adr b){
    if (neValues > MAXEVALUES)
        return;
    eValues[neValues].x1=x1;
    eValues[neValues].y1=y1;
    ...
    for(i=neValues-1;i>=0;i--){
        if(x>=eValues[i].x1 && x<=eValues[i].x2
            ...
            if(selectedEValue.type==0){
                ...
                if(!(selectedEValue.adr == selectedEValue.adrb ))
                    selectedEValue.lastb=getOpt(selectedEValue.adrb);
...
void markSelectedValue(SDL_Surface* screen){
    if(selectedEValue.ptr!=NULL ||selectedEValue.type==3&&correctOpt(selectedEValue.adr)){
        ...
        if(!(selectedEValue.adr == selectedEValue.adrb)){
            drawLine(screen,selectedEValue.x,selectedEValue.y,selectedEValue.x,selectedEValue.my,WHITE);
            drawLine(screen,selectedEValue.x,selectedEValue.my,selectedEValue.mx,selectedEValue.my,WHITE);
...
void modifySelected(int x,int y){
    if(selectedEValue.ptr!=NULL||selectedEValue.type==3&&correctOpt(selectedEValue.adr)){
        ...
        if(selectedEValue.adr == selectedEValue.adrb){
            ...
            d=selectedEValue.last  + ((x-selectedEValue.x))*selectedEValue.d;
            d2=selectedEValue.lastb  + ((selectedEValue.y-y))*selectedEValue.d;
        ...
        if(d2>selectedEValue.max)
                d2=selectedEValue.max;
        if(d2<selectedEValue.min)
                d2=selectedEValue.min;
