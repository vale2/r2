...
void  monitorCorrector(struct cor2*c,char*name){
...
void zeroCorrector(struct cor2*c){
    c->speed=CORMIDLESPEED;
    c->on=0;
    c->gamma=1;
    c->middle=0.5;
}
...
double correctValue(struct cor2*c,double value){
    if(!c->on){
//         if(c->gamma<0.0000001)
  //          c->gamma=0.000001;
...
double correctValue05(struct cor2*c,double value){
     correctValue(c, value);
}
...
void  nearbyCorrect(struct cor2*c1,struct cor2*c2,double d){
    if(c1->gamma<(1.-d)*c2->gamma||
    c2->gamma<(1.-d)*c1->gamma){
        c1->gamma=c1->gamma*0.99+c2->gamma*0.01;
        c2->gamma=c2->gamma*0.99+c1->gamma*0.01;
    }

}
void  nearbyCorrect05(struct cor2*c1,struct cor2*c2,double d){
    if(c1->gamma<(1.-d)*c2->gamma||
    c2->gamma<(1.-d)*c1->gamma){
        c1->gamma=c1->gamma*0.99+c2->gamma*0.01;
        c2->gamma=c2->gamma*0.99+c1->gamma*0.01;
...
double senseValue05(struct sense*s,double v){
       ...
       s->last=v;
       if(s->max<v){
            s->max=s->max*0.99+v*0.01;        
        }
        if(s->min>v){
            s->min=s->min*0.99+v*0.01;        
        }
        if(s->min<s->superMin){
            s->superMin=s->min;
        }
